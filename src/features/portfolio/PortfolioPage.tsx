...
import { TrendingUp, Wallet, History, PieChart, Loader2, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export function PortfolioPage() {
  ...
  const { positions, tradeHistory, totalValue, totalPnl, totalPnlPercent, isLoading, loadPortfolio } = usePortfolioStore();
      ...
      <main className="container py-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <SummaryCard
            ...
            subtext={`${totalPnlPercent >= 0 ? '+' : ''}${totalPnlPercent.toFixed(1)}%`}
          />
          <SummaryCard
            icon={History}
            label="Total Trades"
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="flex items-baseline gap-2">
        ...
        {subtext && (
          <span className={cn('text-sm font-medium', valueClass)}>{subtext}</span>
        )}
      </div>
    </div>
  );
...
function PositionRow({ position }: { position: ReturnType<typeof usePortfolioStore.getState>['positions'][0] }) {
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground truncate">{position.marketTitle}</p>
        <div className="flex items-center gap-2 mt-1">
          <span className={cn(
            'text-xs font-medium px-2 py-0.5 rounded-full',
            position.side === 'yes' ? 'bg-yes-light text-yes' : 'bg-no-light text-no'
          )}>
      ...
      <div className="text-right ml-4">
        ...
        <div className={cn(
          ...
          <span>{isProfit ? '+' : ''}${position.pnl.toFixed(2)} ({position.pnlPercent.toFixed(1)}%)</span>
        </div>
      </div>
    </Link>
  );
...
function TradeRow({ trade }: { trade: ReturnType<typeof usePortfolioStore.getState>['tradeHistory'][0] }) {
  const isBuy = trade.type === 'buy';

  return (
    <div className="flex items-center justify-between p-4">
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground truncate">{trade.marketTitle}</p>
        <div className="flex items-center gap-2 mt-1">
          <span className={cn(
            'text-xs font-medium',
          ...
          <span className={cn(
            'text-xs font-medium px-2 py-0.5 rounded-full',
            trade.side === 'yes' ? 'bg-yes-light text-yes' : 'bg-no-light text-no'
          )}>
