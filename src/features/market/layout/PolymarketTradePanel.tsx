...
export function PolymarketTradePanel({ 
  ...
  const potentialReturn = useMemo(() => {
    ...
    const price = orderType === 'limit' && limitPrice ? parseFloat(limitPrice) / 100 : currentPrice;
  ...
  return (
    <div className="rounded-xl border border-border bg-card sticky top-20">
        ...
        <div className="flex items-center gap-3 p-4 border-b border-border">
          {selectedOutcome.imageUrl ? (
            ...
            <div className="w-10 h-10 rounded-full bg-surface flex items-center justify-center">
              <span className="font-bold text-sm">{selectedOutcome.label.charAt(0)}</span>
            </div>
          )}
          <span className="font-semibold text-foreground">{selectedOutcome.label}</span>
        </div>
      )}
      ...
      <div className="p-4">
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setOrderType('market')}>
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">Execute at best available price</span>
                </div>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setOrderType('limit')}>
                <div className="flex flex-col">
                  <span className="font-medium">Limit</span>
                  <span className="text-xs text-muted-foreground">Set your own price</span>
                </div>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        {orderType === 'limit' && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-foreground mb-2">Limit Price</label>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Order will execute when price reaches {limitPrice || (currentPrice * 100).toFixed(0)}¢
            </p>
          </div>
        )}
          ...
          <div className="space-y-2 mb-4 p-3 rounded-lg bg-surface text-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="text-foreground font-medium">
                {tradeMode === 'buy' ? formatShares(shares) : `$${potentialReturn.toFixed(2)}`}
              </span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn(
                    ...
                    {potentialReturn >= 0 ? '+' : ''}{formatCurrency(potentialReturn)} ({returnPercent >= 0 ? '+' : ''}{returnPercent.toFixed(0)}%)
                  </span>
                </div>
              </>
            )}
      ...
      {showOrderBook && orderBook && (
        <div className="border-t border-border p-4">
          <h4 className="text-sm font-medium text-foreground mb-3">Order Book</h4>
